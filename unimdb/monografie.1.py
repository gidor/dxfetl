...
def template_name():
    return os.path.join(get_main_dir(), "Scheda_template.html")
...
def main():
    ...
    parser.add_argument(
        "-db", dest='dbfile', action='store'  # ,action='append'
        # ~ ,nargs='+'
        , nargs=1, type=str, help='Mdb contenente i dati da esportare nella diriectory del dwg insieme alle immagini di plottaggio')
    parser.add_argument(
        "-f", dest='filter', action='store', nargs=1, type=str, help='filtro pu essere BIANCA NERA ALL o un numero di scheda')
